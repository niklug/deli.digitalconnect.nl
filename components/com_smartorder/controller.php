...
class SmartorderController extends JControllerLegacy
{
    ...
    function registration()
	{
            ...
            elseif (JRequest::getVar('password') <> JRequest::getVar('password2') ) echo outMsg(JText::_('Password error!'), 'error');
                ...
                if (!$user->bind($data)) { // now bind the data to the JUser Object, if it not works....
                    ...
                    if (!$user->save()) { // if the user is NOT saved...
                        ...
                        $mailer->setSender(array($config->get( 'mailfrom' ),
                                            $config->get( 'fromname' )));
                        $mailer->setSubject(JText::_('REGMAIL USER SUBJECT'));
                        $mailer->setBody(JText::sprintf('REGMAIL USER MESSAGE',$name,$email,$password));
        ...
        $view = & $this->getView('Smartorder','html');
        $view->regform();
    }
    ...
    function login(){
        ...
        include(JPATH_COMPONENT.DIRECTORY_SEPARATOR.'smartorder_login.php');
        $mainframe->close();
    }
}