...
jQuery.fn.akosAddNewTopping = function(toppingcat)
{
    return this.each(function()
    {
		...
		$newitem.find(".so-toppingselect").css('width', function(index) {
			return topSelectWidth;
		});	
...
jQuery(document).ready(function() {
    ...
    jQuery(window).scroll(function () { 
		...
		//var lastItemPos = jQuery(".so-oneitem").last().offset().top
...
//alert('task=orderSaveAjax&o='+orderstring+'&uname='+jQuery("#so-contact-1").val()+'&uphone='+jQuery("#so-contact-3").val()+'&uemail='+jQuery("#so-contact-4").val()+'&unote='+jQuery("#so-contact-5").val()+'&uaddress='+jQuery("#so-contact-2").val()+'')
...
/* A Select hoverének hoverintent setupja*/	
/* Nem egyből indul a slide, hanem egy kis tétovázást követően*/
...
function optionHover(){ jQuery(this).descSlider();}
function optionOut(){}
jQuery(".so-itemselect, .so-toppingselect").hoverIntent(hoverIntentsettings);
