...
JSubMenuHelper::addEntry(JText::_('ITEMS'), 'index.php?option=com_smartorder&controller=items&task=list', true );
...
<?php endif; ?>
			...
			<th class="title">
				<?php echo JHTML::_('grid.sort', JText::_('CATEGORY') , 'cat_name', @$this->lists['order_Dir'], @$this->lists['order'] ); ?>
			</th>
        </tr>
    </thead>
    <tfoot>
    ...
    for ($i=0, $n=count( $this->items ); $i < $n; $i++)
    {
  		...
  		$link = JRoute::_( 'index.php?option=com_smartorder&controller=items&task=edit&cid='. $row->id );
        ...
        <tr class="<?php echo "row" . $k; ?>">
			...
			<td>
				<span class="editlinktip hasTip" title="<?php echo JText::_( 'Edit category' );?> :: <?php echo $row->name; ?>">
					...
					<?php echo $row->name.(($row->description > '') ? '<br /><span style="color:#ccc;margin:0">'.$row->description.'</span>' : ''); ?>
					</a>
				</span>
            </td>
<?php if ($this->settings->vat_handling == '1'): ?>
